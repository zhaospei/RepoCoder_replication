...
class Crawler:
    ...
    def search_repo(self) -> List[RepoMetadata]:
        ...
        for page in tqdm(
            ...
            url = f"https://api.github.com/search/repositories?q=language:rust+created:>2023-01-01&sort=star&order=desc&per_page=100&page={page}"
    ...
    def clone_repo(self, repo_urls: list, repo_storage_url: str):
        """Clone repo

        Args:
            repo_urls (list): Github repositories html url
        ...
        for repo_url in tqdm(repo_urls, desc="Cloning repositories"):
            ...
            if not os.path.exists(repo_path):
                os.makedirs(repo_path)  # Create the directory
                subprocess.run(["git", "clone", "--depth", "1", f"https://github.com/{owner}/{repo}.git", repo_path], check=True)
