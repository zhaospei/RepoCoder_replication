...
class RandomHoleDigger:
    def __init__(self, repo_base_dir, repo, context_max_tokens=1000, line_min_tokens=5, max_sample_per_repo=50):
    ...
    def _get_line_types(self, lines):
        ...
        for lineno, line in enumerate(lines):
            ...
            elif stripped_line.startswith(multiline_comment_start):
                in_multiline_comment = True
                line_types[lineno] = 'comment'
            elif stripped_line and stripped_line.startswith(line_comment_start):
                line_types[lineno] = 'comment'
            else:
    ...
    def get_chosen_lines(self):
        ...
        for fpath_tuple, line_no in candidate_lines:
            ...
            line_tokenizer = self.tokenizer.tokenize(line)
            if len(line_tokenizer) > self.context_max_tokens:
                continue
...
if __name__ == '__main__':
    OUT_BASE_DIR = 'output'
    REPO_BASE_DIR = 'rust_benchmark/repositories'
    with open('rust_benchmark/repositories/repo_names.txt', 'r') as f:
        repos = f.read().split('\n')
    # print(repos)
    lines = []
    for repo in repos:
    ...
    Tools.dump_jsonl(lines, os.path.join(OUT_BASE_DIR, 'rust-repos-random-line-completion-non-min-token.jsonl'))